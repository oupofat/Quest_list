...
$(".quest_in_game").on("click", "li", function () {
    ...
    let questFinished = ("<li class='list_quest col-sm-8' value='this is a test' name='finished_quest'> <input class='quest_checked_boxed' type='checkbox' name='working_on'><span class='quest_text'>" + completedQuest + "</span></li>");
...
$(".completed_quest").on("dblclick", ".quest_text", function () {
    //console.log("click")
    $(this).css("color", "blue");
    let test = $(this).closest(".list_quest").append("<input type='text' name='complete' /> <button type='button' name='save'>Save</button>");
    console.log(test);
});


//this is suppose to click the save box and enter the new value
//$(".completed_quest").on("change", function (event) {
//  event.preventDefault();
